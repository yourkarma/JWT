...
@property(nonatomic, copy) NSString *certificateBase64UrlEncodedString;
...
- (void)testDecodeWithBase64UrlEncodedCertificateString {
    JWTBuilder *builder = [JWTBuilder decodeMessage:self.token].secret(self.certificateBase64UrlEncodedString).algorithmName(self.algorithmName);
...
- (void)testDecodeWithBase64UrlEncodedCertificateData {
    NSData *certificateData = [NSData dataWithBase64UrlEncodedString:self.certificateBase64UrlEncodedString];
    JWTBuilder *builder = [JWTBuilder decodeMessage:self.token].secretData(certificateData).algorithmName(self.algorithmName);
    NSDictionary *decodedDictionary = builder.decode;
    [self assertDecodedDictionary:decodedDictionary];
}
